...
def train(loader, model, loss_model, opt, sche, epoch,logger):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        cls_loss = loss_model.weighted_BCE_loss(pred[0],label.cuda(),inc_L_ind.cuda())
        cls_loss2 = loss_model.weighted_BCE_loss(pred[1],label.cuda(),inc_L_ind.cuda())
        if args.alpha>0:
            cont_loss2 = loss_model.contrastive_loss2(x_tran,label.cuda(),inc_V_ind.cuda(), inc_L_ind.cuda())
...
def test(loader, model, loss_model, epoch,logger):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        data=[v_data.to('cuda:0') for v_data in data]
        inc_V_ind = inc_V_ind.float().to('cuda:0')
        pred, _, _ = model(data,mask=inc_V_ind)
        pred = pred[0].cpu()
...
def main(args,file_path):
    ...
    for fold_idx in range(folds_num):
        ...
        model = get_model(len(d_list),d_list,d_model=512,n_layers=6,heads=4,classes_num=train_dataset.classes_num,dropout=0.3,exponent=args.gamma)
        # print(model)
        loss_model = Loss()
        # crit = nn.BCELoss()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folds-num', default=2, type=int)
